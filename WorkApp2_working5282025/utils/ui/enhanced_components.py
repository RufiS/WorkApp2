...
def display_enhanced_answer(answer):
    ...
    # Check if answer is missing required sections
    import re
    required_sections = ["CONTEXT", "QUESTION", "INSTRUCTIONS", "Response"]
    missing_sections = [section for section in required_sections if not re.search(rf'\b{section}\b', answer)]
    
    if missing_sections:
        warning_msg = f"Warning: Answer is missing required sections: {', '.join(missing_sections)}"
