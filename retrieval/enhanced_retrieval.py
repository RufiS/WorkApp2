...
class EnhancedRetrieval:
    ...
    def build_bm25_index(self, chunks: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if invalid_chunks > 0:
                logger.warning(f"Found {invalid_chunks} invalid chunks out of {len(chunks)}")
    ...
    def hybrid_search(
        ...
        try:
            ...
            vec_map = {r["id"]: r["score"] for r in (vector_results or [])}
    ...
    @with_timing(threshold=0.5)
    def mmr_reranking(
        self, query: str, results: List[Dict[str, Any]], top_k: int = 10, lambda_param: float = 0.7
    ) -> List[Dict[str, Any]]:
        """
        ...
        start_time = time.time()
        preview = (query[:50] + "...") if len(query) > 50 else query
        logger.info(f"MMR rerank for '{preview}' with {len(results)} hits, top_k={top_k}")
        if not results:
