...
def safe_highlight_for_streamlit(text: str) -> Dict[str, Any]:
    ...
    """
    result = {"text": text, "uncertain_sections": [], "term_distinctions": [], "warnings": []}
    ...
    if uncertain_count > 0:
        ...
        for match in re.finditer(r"<uncertain>(.*?)</uncertain>", text, re.DOTALL):
            result["uncertain_sections"].append(
                {"text": match.group(1), "start": match.start(), "end": match.end()}
            )
    ...
    confused_term_pairs = [
        (
            r"\b(Revisit)\b.*?\b(billable|paid)\b",
            "Revisit is a complementary service, not billable",
        ),
        (
            r"\b(complementary)\b.*?\b(charge|cost|fee|price)\b",
            "Complementary services should not have charges",
        ),
        ...
        (
            r"\b(Appointment)\b.*?\b(complementary)\b",
            "Be careful about appointment classifications",
        ),
        (
            r"\b(Service)\b.*?\b(appointment)\b.*?\b(same|equivalent|equal)\b",
            "Services and appointments are distinct concepts",
        ),
    ]
