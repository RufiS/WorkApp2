...
def display_uncertain_sections(sections: list) -> None:
    ...
    if sections:
        ...
        for section in sections:
            ...
            st.markdown(get_html_wrapper(content, UNCERTAIN_SECTION_STYLE), unsafe_allow_html=True)
...
def display_term_distinctions(distinctions: list) -> None:
    ...
    if distinctions:
        ...
        for term in distinctions:
            ...
            st.markdown(get_html_wrapper(content, TERM_DISTINCTION_STYLE), unsafe_allow_html=True)
...
def display_answer(answer: str) -> Dict[str, Any]:
    ...
    st.markdown(get_html_wrapper(highlight_data["text"], WRAPPER_DIV_STYLE), unsafe_allow_html=True)
...
def display_debug_info_for_index(doc_processor, query: str) -> None:
    ...
    try:
        ...
        elif hasattr(doc_processor, "index_manager") and hasattr(
            doc_processor.index_manager, "get_stats"
        ):
