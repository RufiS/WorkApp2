...
class ContextEnrichmentProcessor:
    ...
    def __init__(self, debug_mode: bool = False):
        ...
        self.added_elements = {"section_headers": 0, "cross_references": 0, "topic_markers": 0}

    def process(
        self, query: str, topic_clusters: List[List[Dict[str, Any]]]
        ...
        for i, refs in cross_references.items():
            if refs:
                ref_text = "\n\nRELATED INFORMATION:\n" + ", ".join(
                    [f"Section {ref+1}" for ref in refs]
                )
                sections[i] += ref_text
