...
class SemanticGroupingProcessor:
    ...
    def _group_with_embedder(self, query: str, chunks: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        """
        ...
        semantic_groups.sort(key=lambda group: sum(chunk.get("score", 0) for chunk in group) / len(group), reverse=True)
        
        if self.debug_mode:
            logger.debug(f"Created {len(semantic_groups)} semantic groups from {len(chunks)} chunks")
    ...
    def _group_by_consecutive_ids(self, chunks: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        if self.debug_mode:
            logger.debug(f"Created {len(groups)} groups by consecutive IDs from {len(chunks)} chunks")
