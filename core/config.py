...
class ConfigManager:
    ...
    def __init__(
        ...
        performance_config_path: str = "performance_config.json",
    ):
        """
            ...
            try:
                with open(self.config_path, "r") as f:
                    try:
                        ...
                        logger.error(
                            f"Malformed JSON in configuration file {self.config_path}: {str(jde)}"
                        )
                        logger.warning(
                            f"Using default configuration values due to malformed config file"
                        )
                        return app_config
                ...
                for k, v in config_data.items():
                    if k not in ["retrieval", "model", "ui", "performance"] and hasattr(
                        app_config, k
                    ):
            ...
            try:
                with open(self.performance_config_path, "r") as f:
                    try:
                        ...
                        logger.error(
                            f"Malformed JSON in performance configuration file {self.performance_config_path}: {str(jde)}"
                        )
                        logger.warning(
                            f"Using default performance configuration values due to malformed config file"
                        )
                        return app_config
                ...
                logger.error(
                    f"Error loading performance configuration from {self.performance_config_path}: {str(e)}"
                )
...
config_manager = ConfigManager(
    config_path="config.json", performance_config_path="performance_config.json"
)
