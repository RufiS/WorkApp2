...
class TopicClusteringProcessor:
    ...
    def __init__(self, embedder=None, debug_mode: bool = False):
        ...
        self.vectorizer = TfidfVectorizer(
            max_df=0.85, min_df=2, max_features=1000, stop_words="english"
        )
        self.eps = 0.3  # DBSCAN epsilon parameter
        self.min_samples = 2  # DBSCAN min_samples parameter

    def process(
        self, query: str, semantic_groups: List[List[Dict[str, Any]]]
        ...
        try:
            ...
            if self.debug_mode:
                logger.debug(
                    f"Created {len(sorted_clusters)} topic clusters from {len(semantic_groups)} semantic groups"
                )
                for i, cluster in enumerate(sorted_clusters[:3]):
                    logger.debug(
                        f"Cluster {i}: {len(cluster)} chunks, topic: {self.extract_topic(cluster)}"
                    )
    ...
    def _assign_noise_points(
        self, noise_cluster: List[Dict[str, Any]], clusters: List[List[Dict[str, Any]]], query: str
    ) -> None:
        """
                ...
                for noise_chunk in noise_cluster:
                    ...
                    for cluster in clusters:
                        ...
                        cluster_similarity = np.mean(
                            [np.dot(query_embedding, emb) for emb in cluster_embeddings]
                        )
    ...
    def _simple_assign_noise(
        self, noise_cluster: List[Dict[str, Any]], clusters: List[List[Dict[str, Any]]]
    ) -> None:
        """
