...
from core.index_management.index_health import check_index_health
...
def save_index(
    ...
    if chunks_dir and texts and len(texts) > 0:
        try:
            ...
            logger.warning(
                f"Failed to save chunks to {os.path.join(chunks_dir, 'chunks.txt')}: {str(e)}"
            )
    ...
    np.save(
        os.path.join(resolved_index_path, "texts.npy"),
    ...
    _index_cache[cache_key] = index
    _texts_cache[cache_key] = texts
    _last_modified_time[cache_key] = os.path.getmtime(
        os.path.join(resolved_index_path, "index.faiss")
    )
...
def load_index(
    gpu_available: bool = False, index_path: Optional[str] = None, force_reload: bool = False
...
def _load_index_from_disk(
    gpu_available: bool = False, index_path: Optional[str] = None
) -> Tuple[Any, list, int, str]:
    """
                ...
                if not texts or len(texts) == 0:
                    ...
                    if not os.path.exists(chunks_path) and texts and len(texts) > 0:
                        try:
                            ...
                            logger.warning(
                                f"Failed to create chunks file at {chunks_path}: {str(e)}"
                            )
            else:
...
def rebuild_index_if_needed(
    index_path: Optional[str] = None, dry_run: bool = False
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if dry_run:
            return (
                True,
                "[DRY RUN] Index would be reset due to corruption. Documents would need to be re-uploaded.",
            )
        else:
            return (
                True,
                "Index was corrupted and has been reset. Please upload documents to rebuild the index.",
            )
    except Exception as e:
        logger.error(
            f"{'[DRY RUN] ' if dry_run else ''}Failed to rebuild index: {str(e)}", exc_info=True
        )
...
@lru_cache(maxsize=1)
def get_index_stats(
    index_path: Optional[str] = None, texts: Optional[List[str]] = None, index: Any = None
) -> Dict[str, Any]:
    """
    ...
    if os.path.exists(metadata_file):
        try:
            with open(metadata_file, "r") as f:
                ...
                stats["chunk_overlap"] = metadata.get(
                    "chunk_overlap", retrieval_config.chunk_overlap
                )
        except Exception:
            pass
...
def get_saved_chunk_params(index_path: Optional[str] = None) -> Tuple[int, int]:
    ...
    try:
        if os.path.exists(metadata_file):
            ...
            logger.warning(
                f"Metadata file not found at {metadata_file}, using default chunk parameters"
            )
