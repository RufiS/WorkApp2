...
from core.config import app_config, retrieval_config, resolve_path
...
def get_index_stats(
    index_path: Optional[str] = None, texts: Optional[list] = None, index: Optional[Any] = None
) -> Dict[str, Any]:
    """
...
def fix_index(index_path: Optional[str] = None) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not os.path.exists(index_file) or not os.path.exists(texts_file):
            logger.warning(
                f"Index files missing at {resolved_index_path}, creating new empty index"
            )
