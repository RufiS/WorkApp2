...
from core.config import app_config, retrieval_config, performance_config, resolve_path
from core.index_management.index_operations import (
    load_index,
    save_index,
    clear_index,
...
from core.index_management.index_health import check_index_health, has_index
from core.index_management.index_freshness import is_index_fresh
...
class IndexCoordinator:
    ...
    def load(self, force_reload: bool = False, dry_run: bool = False) -> Tuple[bool, str]:
        ...
        if not force_reload and self.is_loaded:
            is_fresh, current_mtime = is_index_fresh(
                index_path=self.index_path, last_known_mtime=self.last_modified_time
            )
            if is_fresh:
        ...
        try:
            ...
            if was_rebuilt:
                logger.warning(
                    f"{'[DRY RUN] ' if dry_run else ''}Index was rebuilt: {rebuild_message}"
                )
            ...
            try:
                ...
                logger.error(
                    f"{'[DRY RUN] ' if dry_run else ''}Error loading index: {str(e)}", exc_info=True
                )
    ...
    def save(self, dry_run: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.error(
                f"{'[DRY RUN] ' if dry_run else ''}Error saving index: {str(e)}", exc_info=True
            )
            return False, f"{'[DRY RUN] ' if dry_run else ''}Failed to save index: {str(e)}"

    def clear(
        self, rebuild_immediately: bool = False, embedder: Any = None, dry_run: bool = False
    ) -> Tuple[bool, str]:
        """
        Clear the index
        ...
        try:
            ...
            logger.error(
                f"{'[DRY RUN] ' if dry_run else ''}Error clearing index: {str(e)}", exc_info=True
            )
    ...
    def update_index(
        self, texts: List[str], embeddings: np.ndarray, append: bool = False, dry_run: bool = False
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if (
                ...
                and embeddings.shape[1] != self.index.d
            ):
                # Dimensions don't match, need to rebuild the index
                logger.warning(
                    f"Embedding dimensions mismatch: index has {self.index.d}, but embeddings have {embeddings.shape[1]}"
                )
                ...
                if (
                    ...
                    if isinstance(self.index, faiss.IndexIVFFlat):
                        ...
                        if not isinstance(nlist, int):
                            raise TypeError(
                                f"Invalid nlist type: {type(nlist).__name__}. Expected integer"
                            )
                        quantizer = faiss.IndexFlatL2(dimension)
                        new_index = faiss.IndexIVFFlat(
                            quantizer, dimension, int(nlist), faiss.METRIC_L2
                        )
                ...
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Created new index with dimension {dimension}"
                )
            ...
            if not append and self.index is not None and hasattr(self.index, "reset"):
                ...
                self.texts = []
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Cleared existing index before adding new data"
                )
            ...
            if append:
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Index updated with {len(texts)} new chunks. Total chunks: {chunk_count}"
                )
            else:
                logger.info(
                    f"{'[DRY RUN] ' if dry_run else ''}Index replaced with {len(texts)} chunks."
                )
            ...
            if dry_run:
                ...
                return (
                    True,
                    f"[DRY RUN] Index updated in memory with {len(texts)} chunks (not saved to disk)",
                )
            else:
    ...
    def search(
        ...
        if indices.size > 0:
            ...
            result_texts = [
                self.texts[idx] if valid else "" for idx, valid in zip(indices[0], valid_indices)
            ]
