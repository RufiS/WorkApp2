        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if exception_types is not None and not any(
                        isinstance(e, exc_type) for exc_type in exception_types
                    ):
                    ...
                    # Log the error
                    logger.warning(
                        f"Attempt {attempt+1}/{max_attempts} failed in {func.__name__}: {str(e)}"
                    )
            ...
            if last_exception is not None:
                ...
                if transform_exception is not None:
                    try:
                        ...
                        if transformed_exception is not None and isinstance(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if propagate_exceptions and any(
                    isinstance(e, exc_type) for exc_type in propagate_exceptions
                ):
                    if log_error:
...
def with_timing(log_level: int = logging.INFO, threshold: Optional[float] = None):
    """
            ...
            try:
                ...
                if log_to_central:
                    try:
                        ...
                        logger.warning(
                            "Could not import log_error function from utils.error_logging"
                        )
                ...
                if suppress_exceptions and any(
                    isinstance(e, exc_type) for exc_type in suppress_exceptions
                ):
